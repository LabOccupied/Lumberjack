...
$SupportLogsPath = Read-Host  -Prompt "`r`n Enter the file path of the Support Files. (Don't include the trailing '\') `n For Example: C:\Cases\1234567\SupportFiles `r`n `r`n Enter File Path"
...
$MPRegistryResults = [System.Collections.ArrayList](Get-Content $MPRegistry | Select-String 'PUAProtection',
'SenseOrgId', 'ASSignatureVersion', 'AVSignatureVersion', ' EngineVersion', ' InstallLocation', 'Current configuration options for location "system policy"')
...
Write-Host '    ' -NoNewLine; Write-Host $i}
   ...
   Write-Host '    Tamper Protection is DISABLED' -BackgroundColor Red }
   ...
   Write-Host '    Tamper Protection is OFF Kill-bitted' -BackgroundColor Red }
...
If ($MpRegistryResults[2].ToString() -Like '*null*'){ 
Try{
Write-Host '    MDE Org ID:                              ' -NoNewline; Write-Host $WdatpOrgID.ToString().substring(62, 36)
} Catch {}
}
